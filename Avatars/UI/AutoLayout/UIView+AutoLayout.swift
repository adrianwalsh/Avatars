...
public extension UIView {
	...
	@discardableResult
	func autoPinEdges(
		toSuperviewEdges edges: [ALEdge],
		withInset inset: CGFloat = 0,
		relation: NSLayoutConstraint.Relation = .equal
	) -> [NSLayoutConstraint] {
		edges.map { autoPinEdge(toSuperviewEdge: $0, withInset: inset, relation: relation) }
	}

	@discardableResult
	func autoPinEdges(toSuperviewMargins edges: [ALMargin]) -> [NSLayoutConstraint] {
		edges.map(autoPinEdge(toSuperviewMargin:))
	}
    ...
    @discardableResult
    func autoSetRatio(to size: CGFloat) -> [NSLayoutConstraint] {
        [
            autoSetDimension(.width, toSize: size),
            autoSetDimension(.height, toSize: size)
        ]
    }
